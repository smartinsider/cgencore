...
#ifndef CGEN_ZCGENWALLET_H
#define CGEN_ZCGENWALLET_H
...
public:
    CzCGENWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZCGEN(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZCGEN(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //CGEN_ZCGENWALLET_H
