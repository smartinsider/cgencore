...
CzCGENWallet::CzCGENWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZCGENSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZCGENSeed_deprecated()) {
                    LogPrintf("%s: Updated zCGEN seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzCGENWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZCGENCount(nCountLastUsed);
    else if (!walletdb.ReadZCGENCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzCGENWallet::Lock()
{
    seedMaster = 0;
}

void CzCGENWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzCGENWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZCGEN(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzCGENWallet::LoadMintPoolFromDB()
{
...
void CzCGENWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzCGENWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzCGENWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzCGENWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZCGEN(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzCGENWallet::SeedToZCGEN(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzCGENWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzCGENWallet::GenerateDeterministicZCGEN(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzCGENWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzCGENWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
