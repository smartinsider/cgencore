...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::CGEN);
    ...
    if (!settings.contains("nAnonymizeCGENcoreAmount"))
        settings.setValue("nAnonymizeCGENcoreAmount", 1000);

    nAnonymizeCGENcoreAmount = settings.value("nAnonymizeCGENcoreAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeCGENcoreAmount"))
        SoftSetArg("-anonymizecgencoreamount", settings.value("nAnonymizeCGENcoreAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in cgencore.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCGENcoreAmount:
            return QVariant(nAnonymizeCGENcoreAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCGENcoreAmount:
            nAnonymizeCGENcoreAmount = value.toInt();
            settings.setValue("nAnonymizeCGENcoreAmount", nAnonymizeCGENcoreAmount);
            emit anonymizeCGENcoreAmountChanged(nAnonymizeCGENcoreAmount);
            break;
        case CoinControlFeatures:
