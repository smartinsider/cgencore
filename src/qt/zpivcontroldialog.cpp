...
std::set<std::string> ZCGENcoreControlDialog::setSelectedMints;
std::set<CMintMeta> ZCGENcoreControlDialog::setMints;

ZCGENcoreControlDialog::ZCGENcoreControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZCGENcoreControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZCGENcoreControlDialog::~ZCGENcoreControlDialog()
{
    delete ui;
}

void ZCGENcoreControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZCGENcoreControlDialog::updateList()
{
...
void ZCGENcoreControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZCGENcoreControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZCGENcore_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZCGENcoreControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZCGENcoreControlDialog::GetSelectedMints()
{
...
void ZCGENcoreControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
