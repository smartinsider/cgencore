...
public:
    ...
    bool WriteZCGENSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZCGENSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZCGENSeed_deprecated(uint256& seed);
    bool EraseZCGENSeed();
    bool EraseZCGENSeed_deprecated();

    bool WriteZCGENCount(const uint32_t& nCount);
    bool ReadZCGENCount(uint32_t& nCount);
