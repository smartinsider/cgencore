...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 CGEN at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zCGEN send
enum ZerocoinSpendStatus {
    ZCGEN_SPEND_OKAY = 0,                            // No error
    ZCGEN_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZCGEN_WALLET_LOCKED = 2,                         // Wallet was locked
    ZCGEN_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZCGEN_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZCGEN_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZCGEN_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZCGEN_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZCGEN_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZCGEN_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZCGEN_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZCGEN_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZCGEN_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZCGEN_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZCGEN_SPENT_USED_ZCGEN = 14,                      // Coin has already been spend
    ZCGEN_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZCGEN_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZCGENcoreBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZCGENOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzCGENWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzCGENTracker> zpivTracker;
    ...
    void setZWallet(CzCGENWallet* zwallet)
    {
        zwalletMain = zwallet;
        zpivTracker = std::unique_ptr<CzCGENTracker>(new CzCGENTracker(strWalletFile));
    }

    CzCGENWallet* getZWallet() { return zwalletMain; }
    ...
    void setZCGENcoreAutoBackups(bool fEnabled)
    {
    ...
    /** zCGEN reset */
    boost::signals2::signal<void()> NotifyzCGENReset;
