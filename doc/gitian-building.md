...
*Setup instructions for a gitian build of CGEN using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the CGEN
...
- [Building CGEN](#building-cgencore)
...
Clone the git repositories for cgencore and gitian and then checkout the cgencore version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/cgencore-crypto/cgencore.git
cd cgencore
...
Follow the instructions in [doc/release-process.md](release-process.md) in the cgencore repository
...
To build CGEN (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the cgencore repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/cgencore/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/cgencore-crypto/cgencore
...
For example:
```bash
URL=https://github.com/crowning-/cgencore.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit cgencore=${COMMIT} --url cgencore=${URL} ../cgencore/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit cgencore=${COMMIT} --url cgencore=${URL} ../cgencore/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit cgencore=${COMMIT} --url cgencore=${URL} ../cgencore/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/cgencore-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/cgencore-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/cgencore-build.assert
```
...
[cgencore/gitian.sigs](https://github.com/cgencore-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
